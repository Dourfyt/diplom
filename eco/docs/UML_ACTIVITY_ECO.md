...
| 3 | Эколог | Запросить экспорт | Выбор формата |
| 4a | Веб-модуль ECO | Подготовить шаблон формата | Параллельная ветка после fork |
| 4b | API платформы | Получить операции за период | `reporting/operations` |
| 5 | Веб-модуль ECO | Объединить данные и сформировать файл | После join |
...
> Для описания динамики работы веб-модуля отчётности построена **UML-диаграмма действий** сквозного сценария экспорта журнала операций (рис. X). Диаграмма разделена на дорожки **«Эколог»**, **«Веб-модуль ECO»** и **«API платформы»**, что отражает распределение ответственности между пользователем, клиентом Django и единым REST API комплекса. После запроса экспорта параллельно выполняются подготовка шаблона отчёта и загрузка операций движения; узел слияния (join) синхронизирует ветки перед формированием файла. Такое представление дополняет BPMN-диаграмму процесса P-14 и функциональную модель IDEF0 блоком A3.
